...
The only function provided by the `unique` package is `unique.[T]Make()` which is a generic function that stores, or **interns**, values in a system-wide "container" for the specific type (ie, `T` the type parameter) and returns an element "handle".  You can call the `Value()` method on a handle to get back the actual value and you can **compare handles**, usually more efficiently than comparing the underlying values.
