...
last_modified_at: 2025-10-04T11:06:00+10:00
excerpt: Go 1.25 has some useful additions, some that are overhyped
toc: true
toc_sticky: true
categories: [language,releases]
tags: [release,synctest,json,gc,waitgroup,GOMAXPROCS,execution tracer,cache,goroutine,thread,scheduling]
header:
...
Here's what I consider are the most significant additions in Go 1.25:
...
* max goroutines is better handled - see [GOMAXPROCS](#gomaxprocs)
* [WaitGroup](#waitgroup) adds a nicety (and gotcha)
...
<font size="+6">❝</font><font size="+3">.. helps you focus on writing better code</font><font size="+6">❞</font><br/>
...
STW pauses are when the Go runtime needs to stop all your goroutines.  They are only required during the GC cycle and a few functions in the `runtime` package (which you should not be calling in production code).
{: .notice--info}
...
I was astonished when Go GC STW pauses were brought down to sub-msec times.  To be honest, I was dubious of Go's GC and for decades avoided GC languages.  The Go GC has had its problems and controversies too, as I discussed in the [Background](#background) section above.

Nowadays, I use Go exclusively and find GC makes my life simpler, generally without noticeable effects on performance.  I thought it couldn't get any better, so I was surprised to see the new GC experiment code named **Green Tea**.
...
Without getting into too much technical detail, the new GC tries to group small heap-allocated objects together (in both space and time :).  This improves use of data and instruction caches.  [Cache coherence](https://en.wikipedia.org/wiki/Cache_coherence) is important especially in the type of highly concurrent software Go is famous for.

<font size="+6">❝</font><font size="+3">... improves use of data & instruction caches ...</font><font size="+6">❞</font><br/>

### Who does it affect?

In most Go software the GC gets a lot of work.  Escape-analysis is good, and the obvious way to write code often leads to more allocations than necessary.

The new GC is supposed to increase garbage collection speed up to 40%.  Unfortunately, this may **not** mean the whole program runs much faster.  I did a simple test and found no measureable speed improvement.
...
1. It determines how many threads are used for goroutines
2. Set at startup from environment variable GOMAXPROCS 
3. Default = estimate of available cores (hardware threads)
4. It can be set dynamically using `runtime.GOMAXPROCS()`

It's the 3rd point that is sometimes a problem because the way that the runtime detects the number of available cores is simplistic.  Moreover, it's set (from the GOMAXPROCS environment variable or default) at startup, and never modified, though it is becoming more common for the number of available CPUs to change dynamically.
...
If you never create any goroutines - never use the `go` keyword, and don't use any packages that do so - then all your code runs in a single goroutine - in this case GOMAXPROCS has no effect on the performance.  (Though extra goroutine(s) may be used for garbage collections.)  However, if you create lots of goroutines it _can_ be important, but I have found that it often does not do much.

The general wisdom is GOMAXPROCS should be no more than the numbers of cores (hardware threads), but low values often have little effect on throughput, though there is an effect on latency for very low values.  It has even been reported that setting GOMAXPROCS higher than recommended can actually improve performance (but this may depend on OS, Go version, etc.).

Note: My experience is limited to server software running on hardware with 20 or more cores.  For example, I run a certain "service" on 4 servers across the world and at times has 3 million or more concurrent goroutines.  It typically only uses 5% CPU (with occasional spikes up to 30%).  I have done lots of experiments, as I can easily (dynamically) control the value of GOMAXPROCS.
{: .notice--info}
...
For example, when my Windows laptop has a low battery, the OS will shut down some cores to save power.

On Linux, processes can be configured to have fewer CPUs available using the `cgroups` facility.  This is used for the `--cpus` docker option, when running Linux containers.

### Go 1.25 Fixes

The changes in Go 1.25 address two related issues:

1. The number of actually _available_ CPUs may be less than the number of hardware cores that the Go runtime believes
2. The number of available CPUs can change _dynamically_

In Go 1.25 the runtime tries to obtain a better estimate of available hardware threads.  Moreover, it will regularly check for changes and adjust the number of OS threads it uses for go-routines dynamically.
...
This will not affect a lot of Go software that already dynamically adjusts `runtime.GOMAXPROCS()`.  For example, a lot of containerised Go software uses the [automaxprocs](https://github.com/uber-go/automaxprocs) open-source package which does this automatically.  For a lot of other Go software it will have no appreciable effect.
...
* execution tracer - records exact behaviour of your goroutines and their interactions + use of the garbage collector, etc
...
So, what is the difference between profiling and tracing?  A crucial difference is that profiling uses sampling, while the execution tracer records everything.
{: .notice--info}

### Execution Tracer

With so many test tools the execution tracer often gets overlooked.  This is a shame since it works so well now - see my blog on Go 1.22 [Execution Tracer](/blog/go1p22.html#execution-tracer)

Since using Go 1.22 I often turn it on in production in order to understand some problem.  The extra CPU overhead of recording a trace is small but if you record for a long time it will use a lot of memory.

### Why Do I Need It?

In a career of 45+ years (mainly C and C++) I have encountered lots of mysterious runtime issues (crashes, slowness, etc).  Luckily, Go makes it orders of magnitude easier to create understandable software, but sometimes there are still things that are tricky to understand.
...
```go
    const MaxTraceBytes = 10_000_000
    recorder := trace.NewFlightRecorder(trace.FlightRecorderConfig{MaxBytes: MaxTraceBytes})
...
This code will use a trace buffer not exceeding 10Mbytes.  Note that you can also restrict the trace length to a number of seconds, but it's simpler to just specify the amount of memory reserved for the circular buffer.

You can also set the trace length (in seconds) but the `trace` package gives a sensible default, and it _never exceeds the memory limit_ you give it.
{: .notice--warning}
...
```go
    if traceWantedForSomeReason() {
		filename := "trace_" + time.Now().UTC().Format("060102150405") + ".out"
...
One of the best parts of execution traces is the built-in tooling. The easiest way to inspect a trace:

$ go  tool  trace  traceFileName.out
...
Plenty has been written about it already (e.g. see [Go Blog](https://go.dev/blog/jsonv2-exp), [JSON evolution in Go: from v1 to v2](https://antonz.org/go-json-v2/), etc) so here are my highlights:
...
This is my favourite.  I often find I have to grab some JSON, modify part of it, then put it back where I got it.  The main difficulty is it is hard to process the JSON you understand while preserving the rest.

As an example, imagine I have a JSON object which I know has a "name" field but may have other **unknown** fields.  These could be used for some other purpose, or be added in the future and need to be kept, for forward-compatibility.
...
Unfortunately, this loses any JSON fields we don't know about.  The JSON after the round trip is just:
...
This looks almost as simple, but it gets messy for larger objects — and I try to avoid using interfaces (esp. `any`) and type assertions for type safety.

With JSON v2 we can just add any extra field (called `Other` in the code below) with the JSON `unknown` tag:
...
One interesting development in Go 1.25 is that the original json package has been re-implemented using the new JSON packages.  So you might see some differences, such as performance improvements and even bugs, but only (for now) if you turn on the `jsonv2` experiment.

### Recommendation

To help the Go Creators finding bugs you might like to run all your JSON-related tests under Go 1.25 with the experiment turned on.  This may reveal bugs in the new packages or the changes to the original JSON package.
...
A newbie mistake is **not calling `Add()` in the correct place**.  For example:
...
Go 1.25 finally adds errgroup's `Go()` function to the standard library.  I've preferred `errgroup` for years — this might finally bring me back to the standard library.
...
In the Go 1.24 experiment you'd call `synctest.Run(f)`.  This takes one parameter (of type `func()`) - the function to be run inside the bubble.
...
In Go 1.25, this has been deprecated in favour of `synctest.Test(t, f)` which has an additional `*testing.T` parameter.  (This was done for a minor technical issue which I'm not sure I agree with, since it restricts synctest to only be used with tests.)
...
One comment I got about my synctest examples is that I did not demonstrate the use of `synctest.Wait()`.  I could not find a good reason to use `synctest.Wait()` -- for external tests, normal Go synchronisation (like `WaitGroup`) does it better.

Since then, I have seen some valid uses of synctest.Wait() to verify conditions/state in _internal_ tests, but I **frown on internal tests**.

## Conclusion

It's great that `synctest` has migrated from **experiment** status, but there are some other great things in Go 1.25.  The new Green Tea GC **experiment** is more _cache friendly_, which is always a good thing, but we'll have to wait to see how effective it is.

The `GOMAXPROCS` and `sync.WaitGroup` changes aren't particularly exciting, but consolidating them into the standard library is still a net positive — one less external dependency to manage.

The new JSON/v2 package, on the surface, does not look that useful, but it has a myriad of improvements in efficiency, reliability, security, compatibility, verifiability and usability.  I especially appreciate the new `unknown` tag.  It's also nice to see a major improvement to Go that comes from the general Go community (outside Google).

It's also interesting to see that they have re-implemented the original JSON (v1) package in terms of the new (v2) package.  This means you will get JSON decoding performance improvements without any code changes (but you have to turn on the experiment in Go 1.25).

I'm very excited by the Trace Flight Recorder.  I previously discussed the [Execution Tracer](/blog/go1p22.html#execution-tracer) improvement in Go 1.22, but due to the unwieldiness of traces it was hard to catch rarely occurring events in production. The `runtime/trace.FlightRecorder` package allows you to trigger a trace capture of the last few seconds at any time.

Remember that to help improve Go 1.26 run your tests on code built with Go 1.25 and the experiments turned on (`GOEXPERIMENT=greenteagc,jsonv2`).  You don't need to make any code changes but don't forget that _production_ code should **not** be built with the experiments on.  Please report any issues to the [Go Issue Tracker](https://github.com/golang/go/issues).
{: .notice--info}
