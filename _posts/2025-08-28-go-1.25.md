...
* max goroutines i better handled - see [GOMAXPROCS](#gomaxprocs)
...
With all these different tools the execution tracer often gets overlooked.  This is a shame since it now works so well - see my blog on Go 1.22 [Execution Tracer](/blog/go1p22.html#execution-tracer)
...
This is my favourite.  I often find I have to grab some JSON, modify part of it, then put it back where I got it.  The main difficulty is it is hard to process the JSON you understand while preserving the rest of it.
...
This looks almost as simple but for larger objects gets messy.  I avoid using interfaces (esp. `any`) and type assertions for type safety, etc.
...
In Go 1.25, this has been deprecated in favour of `synctest.Test(t, f)` which additionally takes a `*testing.T` parameter.  (This was done for a minor technical issue which I'm not sure I agree with, since it restricts synctest to only be used with tests.)
...
One comment I got about my synctest examples is that I did not demonstrate the use of `synctest.Wait()`.  Unfortunately, I could not find a good reason to use `synctest.Wait()`.  IMHO it's better, for external tests, to use the normal Go synchronisation facilities (like `WaitGroup`).
