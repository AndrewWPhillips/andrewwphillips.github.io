...
It's inherently difficult, in C, to ensure that everything is cleaned up in all code paths.  It's _impossible_ to ensure that future modifications do the same.
...
Note that this operates at the `try` statement level, not the function level as in Go.  Since `try` is normally only used with exceptions many Java developers think this is only for use with **exceptions**, but it can actually be used even in the absence of exceptions.)

The only problem is that the "deallocate" code becomes separated from the "allocate" code by the body of the `try` statement.
---
</details>
<!--****************************-->
<details markdown="1">
...
Not being an OO language, Go cannot use RAII because there are no constructors and destructors.  Luckily, Go has full support for functional programming, which allowed the simple solution of the `defer` statement.  In fact, I find `defer` more flexible than RAII as it can be called conditionally and is not tied to the lifetime of a local variable.

Go also handles "exceptions" (ie, `panic`) more cleanly than C++ and other languages.  For example, it can be difficult to follow, and clean-up, when an exception is thrown in a constructor or inside a destructor which is being called in the context of an exception.
...
The basics of `defer` have been covered in other fine blogs such as [Demystifying 'defer'](https://bitfieldconsulting.com/golang/defer) so I won't go over the basic.  (If you don't know what I mean by _basics_ then see the above **Background -> Go** section.)
...
Here's anothet scenario... Sometimes you may be deep in a function, such as a `for` loop or `switch` and need to allocate and release some resource.  The resource has to be released immediately, instead of when the function returns.
