...
I looked at the `unique` package in my [Go 1.23](https://andrewwphillips.github.io/blog/go1p23.html#unique-package) post.  I've decided to look at it in depth as some gophers have dismissed it as pointless thinking that maps or pointers can do the same thing, and some are not really worried about saving a bit of memory.  They are missing out.
...
The Go standard library has had a such a package, for internal use, since [Go 1.20](https://github.com/golang/go/blob/release-branch.go1.20/src/internal/intern/intern.go).  This was _in turn_ based on https://github.com/go4org/intern.
...
Then, why not just use simple pointers?  When objects have the same value use pointers to the same object.

This is what the `unique` package does, but it relieves you of the burden of tracking who _owns_ the uniquw copy of the object.
...
We make the reasonalble assumption here that voucher strings come from "external" sources such as a database, user entry, etc, and are not subject to string interning as explained in the [Background->Go](#background) section above.
{: .notice--info :}
...
Although the problem of substrings was addressed (because `unique.Make()` will "clone" strings as mentioned above), there is still a problem when an interned object refers to part of a larger object on the heap.  This is demonstrated by the following:
...
On first use, I was surprised that the `unique` package uses "global" data structures.  (Internally, it uses a single global map of maps.)
...
If this bothers you, you can obtain your own private "set" of values by creating a new type.  For example, use `type myString string`, in order to get a different container when you use `unique.Make(myString("abc"))`.
