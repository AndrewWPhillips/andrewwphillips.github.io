...
tags: [release,execution tracer,for,range,capture]
header:
...
Or maybe it was done like that as a learning tool :).  Once you encounter the gotcha, you learn the hard way, that variables are **captured by reference**!  I found this a useful lesson as I started to use Go's great functional programming features.
{: .notice--warning}

This problem seems to affect Go newbies more than users of other languages.  I suspect that this is because of how much easier it is to use concurrency -- new Gophers will start a new goroutine in a loop something like this:
