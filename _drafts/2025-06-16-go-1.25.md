...
* I covered [synctest](#synctest) in detail [last time](https://andrewwphillips.github.io/blog/go1p24.html#new-synctest-package). I also did a [talk](https://www.youtube.com/watch?v=KAImJOqagXY)
* There's a [new Garbage Collector](#green-tea-gc) which is more cache friendly
...
* [JSON v2 package](#json-v2) decodes faster and adds one useful thing
* [Trace Flight Recorder](#trace-flight-recorder) is a really cool addition that allows continuous trace recording (ring buffer) so you can dump a trace of the last few seconds as soon as something __interesting__ happens

After trying these things in depth I found several really useful: synctest, trace flight recorder, and the new JSON v2 `unknown` tag.

Some other things sound good on paper, but may have been overhyped by some bloggers: new WaitGroup `Go()` method, JSON v2 in general, `GOMAXPROCS` tweaks.  But **please correct me if I have made an error of judgement**.
